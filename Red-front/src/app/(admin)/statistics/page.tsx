...
export default function StatisticsPage() {
  ...
  const fetchStatistics = async (
    ...
    sortF?: keyof typeof sortOptions,
    sortO?: "asc" | "desc"
  ) => {
    setLoading(true);

    // Use current state values if parameters are undefined
    const actualSortField = sortF !== undefined ? sortF : sortField;
    const actualSortOrder = sortO !== undefined ? sortO : sortOrder;

    try {
      const params: any = {
        page: 1,
        size: 9999,
        sort: sortOptions[actualSortField][actualSortOrder],
      };
